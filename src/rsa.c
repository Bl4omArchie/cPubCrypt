...
int generate(int key_size) {
    ...
    */
    ...
    //As a simplification I gave the public exponent the value 65537 
    struct RSA_KEYPAIR kp = { .key_size=key_size, .public_exponent=65537 };

        
    //generation of prime factors p and q
    mpz_t *p = &kp.p_factor;
    mpz_t *q = &kp.q_factor;
    get_prime_factors(&p, &q, key_size);
